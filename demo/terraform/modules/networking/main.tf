# Networking module: VPC with public/private subnets across 2 AZs, IGW, a NAT
# gateway, S3/DynamoDB gateway endpoints (free), and security groups.
# Mirrors the production network layer (NFR-1 multi-AZ; SC-6 isolation).
...
# Single NAT gateway for the demo (production uses one per AZ - see omissions).
...
# Gateway VPC endpoints (free) so S3/DynamoDB traffic avoids the NAT path.
...
resource "aws_security_group" "elasticache" {
  ...
  tags = { Name = "${var.name_prefix}-elasticache-sg" }
}
